##this is a continuation from the adjoining c,mm r code
...
result_mm_inc_age2_dep1<-ttt %>% ##new conditions per year
  group_by(adm_year,dep_sc,ageg) %>% 
  summarize(number= n())

ttt<-filter(temp_total, ageg=="50_59" & dep_sc=="5")
result_mm_inc_age2_dep5<-ttt %>% ##new conditions per year
  group_by(adm_year,dep_sc,ageg) %>% 
  summarize(number= n())


ttt<-filter(temp_total, ageg=="60_69" & dep_sc=="1")
result_mm_inc_age3_dep1<-ttt %>% ##new conditions per year
  group_by(adm_year,dep_sc,ageg) %>% 
  summarize(number= n())

ttt<-filter(temp_total, ageg=="60_69" & dep_sc=="5")
result_mm_inc_age3_dep5<-ttt %>% ##new conditions per year
  group_by(adm_year,dep_sc,ageg) %>% 
  summarize(number= n())


ttt<-filter(temp_total, ageg=="70_79" & dep_sc=="1")
result_mm_inc_age4_dep1<-ttt %>% ##new conditions per year
  group_by(adm_year,dep_sc,ageg) %>% 
  summarize(number= n())

ttt<-filter(temp_total, ageg=="70_79" & dep_sc=="5")
result_mm_inc_age4_dep5<-ttt %>% ##new conditions per year
  group_by(adm_year,dep_sc,ageg) %>% 
  summarize(number= n())
...
mm_inc_yearly<-cbind.data.frame(result_mm_inc_all,result_mm_inc_sex_f,result_mm_inc_sex_m,result_mm_inc_age1,
                                ...
                                result_mm_inc_age2_dep1, result_mm_inc_age3_dep1,
                                result_mm_inc_age4_dep1, result_mm_inc_age2_dep5,
                                result_mm_inc_age3_dep5,   result_mm_inc_age4_dep5,
                                 result_mm_inc_age5_dep1,result_mm_inc_age5_dep5)
mm_inc_yearly_age1_dep55<-cbind.data.frame(result_mm_inc_age1_dep5)
...
rm(result_mm_inc_all,result_mm_inc_sex_f,result_mm_inc_sex_m,result_mm_inc_age1,
   ...
   result_mm_inc_agem2,result_mm_inc_agem3,result_mm_inc_agem4,result_mm_inc_agem5,
   result_mm_inc_depm1,result_mm_inc_depm5,result_mm_inc_age1_dep1,
   result_mm_inc_age1_dep5,result_mm_inc_age2_dep1, result_mm_inc_age3_dep1,
   result_mm_inc_age4_dep1, result_mm_inc_age2_dep5,
   result_mm_inc_age3_dep5,   result_mm_inc_age4_dep5,
   result_mm_inc_age5_dep1,result_mm_inc_age5_dep5)
fwrite(mm_inc_yearly, file="mm_inc_yearly_april_2022.csv",sep=",")
fwrite(mm_inc_yearly_age1_dep5, file="mm_inc_yearly_age1_dep5_april_2022.csv",sep=",")