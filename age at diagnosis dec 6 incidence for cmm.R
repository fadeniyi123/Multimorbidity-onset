...
unique(all_elix_pheno$Elixhauser_Index_Phenotypes)
##clean up the phenotypes
tt<-filter(all_elix_pheno,Elixhauser_Index_Phenotypes=="Pulmonary Circulation Disorders + Chronic Pulmonary Disorders") length(unique(tt$MAIN_CONDITION))
unique(tt$MAIN_CONDITION)
tt<-filter(all_elix_pheno,Elixhauser_Index_Phenotypes=="Liver Disease + Alcohol Abuse")
unique(tt$MAIN_CONDITION)
all_elix_pheno$Elixhauser_Index_Phenotypes[all_elix_pheno$Elixhauser_Index_Phenotypes=="Liver Disease + Alcohol Abuse"]<-"Alcohol Abuse"
...
#create data caputrue date and obtain age at diagnosis
all_elix_pheno_d$capture_date<-"2019-01-01"
all_elix_pheno_d$capture_date<-ymd(all_elix_pheno_d$capture_date)
all_elix_pheno_d$ADMISSION_DATE<-ymd(all_elix_pheno_d$ADMISSION_DATE)
...
mm_cohort_data2$diag_age3<-coalesce(mm_cohort_data2$diag_age2,mm_cohort_data2$diag_age)##replace mising values in diag_age2 with diag_age
mm_cohort_data2<-select(mm_cohort_data2,PROCHI,dob,cond,ADMISSION_DATE, date_of_death,Elixhauser_Score,Elixhauser_Index_Phenotypes, diag_age, diag_age2,diag_age3, sex, ageg, dep_sc)
...
###add year of death
mm_cohort_data2$death_year<-as.Date(mm_cohort_data2$date_of_death)
mm_cohort_data2$death_year<-format(mm_cohort_data2$death_year, "%Y")
#mm_cohort_data2<-distinct(mm_cohort_data2, PROCHI, .keep_all=TRUE)
mm_cohort_data2$diag_age2<-as.integer(mm_cohort_data2$diag_age2)

fwrite(mm_cohort_data2, file = "inc_death_diag_age_data.csv", sep=",")     


###add year of death to mm_cohort2 -the unique data
mm_cohort2$death_year<-as.Date(mm_cohort2$date_of_death)
mm_cohort2$death_year<-format(mm_cohort2$death_year, "%Y")
#mm_cohort_data2<-distinct(mm_cohort_data2, PROCHI, .keep_all=TRUE)
...
result_mm_all<-mm_cohort_data2 %>% 
  group_by(Elixhauser_Index_Phenotypes) %>% 
  summarize(median_age= median(diag_age3),mean_age= round(mean(diag_age3),1),
            min_age= min(diag_age3), max_age= max(diag_age3))

result_mm_sex<-mm_cohort_data2 %>% 
  group_by(Elixhauser_Index_Phenotypes,sex) %>% 
  summarize(median_age= median(diag_age3),mean_age= round(mean(diag_age3),1),
            min_age= min(diag_age3), max_age= max(diag_age3))

result_mm_agegroups<-mm_cohort_data2 %>% 
  group_by(Elixhauser_Index_Phenotypes,ageg) %>% 
  summarize(median_age= median(diag_age3),mean_age= round(mean(diag_age3),1),
            min_age= min(diag_age3), max_age= max(diag_age3))

result_mm_sex_age<-mm_cohort_data2 %>% 
  group_by(Elixhauser_Index_Phenotypes,sex,ageg) %>% 
  summarize(median_age= median(diag_age3),mean_age= round(mean(diag_age3),1),
            min_age= min(diag_age3), max_age= max(diag_age3))

result_mm_depriv<-mm_cohort_data2 %>% 
  group_by(Elixhauser_Index_Phenotypes, dep_sc) %>% 
  summarize(median_age= median(diag_age3),mean_age= round(mean(diag_age3),1),
            min_age= min(diag_age3), max_age= max(diag_age3))

result_mm_sex_depriv<-mm_cohort_data2 %>% 
  group_by(Elixhauser_Index_Phenotypes,sex,dep_sc) %>% 
  summarize(median_age= median(diag_age3),mean_age= round(mean(diag_age3),1),
            min_age= min(diag_age3), max_age= max(diag_age3))

#age_at_diagnosis=as.data.frame(cbind(result_mm_mean, result_mm_median[,2], result_mm_max[,2],result_mm_min[,2]))
fwrite(result_mm_all, file = "age_at_diagnosis_all_mm.csv", sep=",")
fwrite(result_mm_sex, file = "age_at_diagnosis_sex_mm.csv", sep=",")
fwrite(result_mm_agegroups, file = "age_at_diagnosis_age_mm.csv", sep=",")
fwrite(result_mm_sex_age, file = "age_at_diagnosis_sex_age_mm.csv", sep=",")
fwrite(result_mm_depriv, file = "age_at_diagnosis_depriv_mm.csv", sep=",")
fwrite(result_mm_sex_depriv, file = "age_at_diagnosis_sex_depriv_mm.csv", sep=",")
#rm(result_mm_mean, result_mm_median, result_mm_max,result_mm_min)
rm(result_mm_agegroups,result_mm_all, result_mm_depriv, result_mm_sex, result_mm_sex_age,result_mm_sex_depriv)
...
length(unique((mm_cohort_data$PROCHI)))
length(unique((mm_cohort_data$PROCHI)))
length(which(is.na(mm_cohort_data$dob)))
length(which(!is.na(mm_cohort_data$dob)))
...
all_elix_pheno_d$year<-format(all_elix_pheno_d$year, "%Y")
...
per_year_inc_all<-all_elix_pheno_d %>% 
  group_by(year, Elixhauser_Index_Phenotypes) %>% 
  summarise(n())
...
cmm_inc_yearly_dep1_age5<-cbind.data.frame(result_cmm_inc_age1_dep5)
...
rm(result_cmm_inc_all,result_cmm_inc_sex_f,result_cmm_inc_sex_m,result_cmm_inc_age1,
   ...
   result_cmm_inc_agem2,result_cmm_inc_agem3,result_cmm_inc_agem4,result_cmm_inc_agem5,
   result_cmm_inc_depm1,result_cmm_inc_depm5,result_cmm_inc_age1_dep1,
   result_cmm_inc_age1_dep5,result_cmm_inc_age5_dep1,result_cmm_inc_age5_dep5)
fwrite(cmm_inc_yearly, file="cmm_inc_yearly.csv",sep=",")
fwrite(cmm_inc_yearly_dep1_age5, file="cmm_inc_yearly_dep1_age5.csv",sep=",")
